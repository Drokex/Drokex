...
import PqrEmail from "@/emails/PqrEmail";
...
export async function sendPqrEmail({ name, email, type, message }) {
  const html = await render(PqrEmail({ name, email, type, message, siteUrl }));
  const subject = `[PQR] ${type} de ${name}`;
  const to = "info@drokex.com";
  ...
  if (process.env.RESEND_API_KEY) {
    try {
      ...
      if (error) console.error("[email] sendPqrEmail (Resend) falló:", error.message || error);
      return;
    } catch (e) {
      console.error("[email] sendPqrEmail (Resend) falló:", e?.message);
      return;
    }
  }
  ...
  try {
    ...
    console.error("[email] sendPqrEmail falló:", e?.message);
  }
}
