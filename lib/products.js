import { prisma } from "@/lib/prisma";
import {
  availabilityOptions,
  categoryOptions,
  sampleProducts,
} from "@/lib/sample-products";
import { formatMoney, inferCurrencyFromOriginCountry } from "@/lib/market-pricing";

// Caché en memoria: evita consultar Supabase en cada request
const _cache = globalThis.__drokexProductsCache ?? { data: null, ts: 0 };
if (!globalThis.__drokexProductsCache) globalThis.__drokexProductsCache = _cache;
const CACHE_TTL = 60_000; // 60 segundos

function withDbTimeout(promise, ms = 8000) {
  return Promise.race([
    promise,
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error("DATABASE timeout")), ms)
    ),
  ]);
}

function slugify(value) {
  return value
    .toLowerCase()
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "");
}

const DATA_IMAGE_RE = /^data:image\/(png|jpe?g|webp|gif|avif);base64,/i;

function normalizeImage(value) {
  const image = (value || "").trim();

  if (image.startsWith("data:")) {
    return DATA_IMAGE_RE.test(image) ? image : "";
  }

  if (
    image.startsWith("/") ||
    image.startsWith("http://") ||
    image.startsWith("https://")
  ) {
    return image;
  }

  return "";
}

function normalizeTextList(values) {
  return (values || []).map((item) => (item || "").trim()).filter(Boolean);
}

function normalizeSpecs(specs) {
  if (!Array.isArray(specs)) return [];

  return specs
    .map((item) => {
      if (!item || typeof item !== "object") return null;
      const label = typeof item.etiqueta === "string" ? item.etiqueta.trim() : "";
      const value = typeof item.valor === "string" ? item.valor.trim() : "";
      if (!label || !value) return null;
      return { etiqueta: label, valor: value };
    })
    .filter(Boolean);
}

function inventoryTone(stock, minimumStock) {
  if (stock <= 0) return "out-of-stock";
  if (stock <= minimumStock) return "low-stock";
  return "in-stock";
}

function toStoreProduct(product) {
  const priceCurrency = inferCurrencyFromOriginCountry(product.originCountry);

  return {
    id: product.id || product.slug,
    slug: product.slug,
    sku: product.sku || "",
    supplier: product.supplier,
    originCountry: product.originCountry,
    category: product.category,
    name: product.name,
    priceCurrency,
    priceValue: product.price,
    previousPriceValue: product.previousPrice,
    price: formatMoney(product.price, priceCurrency),
    previousPrice: formatMoney(product.previousPrice, priceCurrency),
    stock: product.stock,
    minimumStock: product.minimumStock,
    inventoryState: inventoryTone(product.stock, product.minimumStock),
    image: product.id && normalizeImage(product.image)?.startsWith("data:")
      // Cache-buster con updatedAt: la ruta de imagen cachea agresivo (24h) y sin esto
      // el navegador seguía mostrando la foto vieja después de guardar una nueva.
      ? `/api/products/${product.id}/image?v=${product.updatedAt ? new Date(product.updatedAt).getTime() : 0}`
      : normalizeImage(product.image),
    galleryImages: normalizeTextList(product.galleryImages).map(normalizeImage).filter(Boolean),
    video: product.video || "",
    availability: product.availability,
    shortDescription: product.shortDescription,
    description: product.description,
    application: product.application || "",
    marketFocus: product.marketFocus || "",
    compatibility: normalizeTextList(product.compatibility),
    technicalSpecs: normalizeSpecs(product.technicalSpecs),
    featured: Boolean(product.featured),
    active: product.active !== false,
  };
}

function canUseSampleProducts(error) {
  return error instanceof Error && /ENOTFOUND|ECONN|tenant\/user|DATABASE/i.test(error.message);
}

export function getSampleStoreProducts() {
  return sampleProducts.map(toStoreProduct);
}

export async function getProducts() {
  // Devolver caché si está vigente
  if (_cache.data && Date.now() - _cache.ts < CACHE_TTL) {
    return _cache.data;
  }

  if (!prisma) {
    return getSampleStoreProducts();
  }

  try {
    const records = await withDbTimeout(
      prisma.product.findMany({
        where: { active: true },
        orderBy: [{ featured: "desc" }, { createdAt: "desc" }],
      })
    );
    const products = records.map(toStoreProduct);
    _cache.data = products;
    _cache.ts = Date.now();
    return products;
  } catch (error) {
    if (!canUseSampleProducts(error)) throw error;
    return getSampleStoreProducts();
  }
}

const _adminCache = globalThis.__drokexAdminProductsCache ?? { data: null, ts: 0 };
if (!globalThis.__drokexAdminProductsCache) globalThis.__drokexAdminProductsCache = _adminCache;

export async function getAdminProducts() {
  if (_adminCache.data && Date.now() - _adminCache.ts < CACHE_TTL) {
    return _adminCache.data;
  }

  if (!prisma) {
    return getSampleStoreProducts();
  }

  try {
    const records = await withDbTimeout(
      prisma.product.findMany({
        orderBy: [{ featured: "desc" }, { createdAt: "desc" }],
      })
    );

    const products = records.map(toStoreProduct);
    _adminCache.data = products;
    _adminCache.ts = Date.now();
    return products;
  } catch (error) {
    if (!canUseSampleProducts(error)) throw error;
    return getSampleStoreProducts();
  }
}

export async function getProductsByProvider(providerId) {
  if (!prisma) {
    return getSampleStoreProducts();
  }

  try {
    const records = await prisma.product.findMany({
      where: { providerId },
      orderBy: [{ featured: "desc" }, { createdAt: "desc" }],
    });

    return records.map(toStoreProduct);
  } catch (error) {
    if (!canUseSampleProducts(error)) throw error;
    return getSampleStoreProducts();
  }
}

export async function getProductBySlug(slug) {
  const products = await getProducts();
  return products.find((item) => item.slug === slug) || null;
}

function normalizeProductInput(input) {
  const name = (input.name || "").trim();
  const slug = (input.slug || "").trim() || slugify(name);

  if (!name) {
    throw new Error("El nombre es obligatorio.");
  }

  return {
    slug,
    sku: (input.sku || "").trim() || null,
    supplier: (input.supplier || "").trim() || "Drokex Supplier",
    originCountry: (input.originCountry || "").trim() || "Colombia",
    category: categoryOptions.includes(input.category) ? input.category : categoryOptions[0],
    name,
    price: Number(input.priceValue || 0),
    previousPrice: Number(input.previousPriceValue || input.priceValue || 0),
    stock: Number(input.stock || 0),
    minimumStock: Number(input.minimumStock || 0),
    image: normalizeImage(input.image),
    galleryImages: normalizeTextList(input.galleryImages).map(normalizeImage).filter(Boolean),
    video: (input.video || "").trim() || null,
    availability: availabilityOptions.includes(input.availability)
      ? input.availability
      : availabilityOptions[0],
    shortDescription: (input.shortDescription || "").trim(),
    description: (input.description || "").trim(),
    application: (input.application || "").trim() || null,
    marketFocus: (input.marketFocus || "").trim() || null,
    compatibility: normalizeTextList(input.compatibility),
    technicalSpecs: normalizeSpecs(input.technicalSpecs),
    featured: Boolean(input.featured),
    active: input.active !== false,
  };
}

function invalidateProductCaches() {
  _cache.data = null;
  _cache.ts = 0;
  _adminCache.data = null;
  _adminCache.ts = 0;
}

export async function createProduct(input, providerId) {
  const payload = { ...normalizeProductInput(input), providerId: providerId || null };

  if (!prisma) {
    return toStoreProduct(payload);
  }

  const created = await prisma.product.create({
    data: payload,
  });

  await prisma.inventoryMovement.create({
    data: {
      productId: created.id,
      type: "CREATED",
      quantity: created.stock,
      stockAfter: created.stock,
      note: "Producto creado desde el panel Drokex.",
    },
  });

  invalidateProductCaches();
  return toStoreProduct(created);
}

export async function updateProduct(id, input) {
  if (!prisma) {
    return toStoreProduct({ id, ...normalizeProductInput(input) });
  }

  // El listado admin proxya imágenes base64 pesadas como "/api/products/{id}/image"
  // para no viajar el blob completo en cada request. Si ese string vuelve tal cual
  // en un guardado, no es una imagen real — habría que reescribir el original.
  const sanitizedInput = { ...input };
  if (typeof sanitizedInput.image === "string" && /^\/api\/products\/.+\/image$/.test(sanitizedInput.image)) {
    const existing = await prisma.product.findUnique({ where: { id }, select: { image: true } });
    sanitizedInput.image = existing?.image || "";
  }

  const payload = normalizeProductInput(sanitizedInput);

  const updated = await prisma.product.update({
    where: { id },
    data: payload,
  });

  invalidateProductCaches();
  return toStoreProduct(updated);
}

export async function getInventoryMovements() {
  if (!prisma) {
    return [];
  }

  let records;
  try {
    records = await prisma.inventoryMovement.findMany({
      orderBy: { createdAt: "desc" },
      take: 12,
      include: { product: true },
    });
  } catch (error) {
    if (!canUseSampleProducts(error)) throw error;
    return [];
  }

  return records.map((record) => ({
    id: record.id,
    productName: record.product.name,
    productSlug: record.product.slug,
    type: record.type,
    quantity: record.quantity,
    stockAfter: record.stockAfter,
    note: record.note,
    createdAt: record.createdAt,
  }));
}

export async function deleteProduct(id) {
  if (!prisma) return;
  await prisma.product.delete({ where: { id } });
  invalidateProductCaches();
}

export { availabilityOptions, categoryOptions };
