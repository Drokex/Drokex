...
export async function getQuotesForProvider(providerId) {
  const quotes = await prisma.quote.findMany({
    where: providerId ? { product: { providerId } } : undefined,
    include,
    orderBy: { createdAt: "desc" },
  });
  return quotes.map(formatQuote);
  ...
  try {
    await getTransporter().sendMail({
      ...
      html: `
        ...
        <p><strong>Producto:</strong> ${escapeHtml(quote.product?.name)}</p>
        <p><strong>Cliente:</strong> ${escapeHtml(quote.client?.fullName)} (${escapeHtml(quote.client?.email)})</p>
        <p><strong>Cantidad:</strong> ${quote.quantity}</p>
        <p><strong>Destino:</strong> ${escapeHtml(quote.destinationCountry)}</p>
        <p><strong>Mensaje:</strong> ${escapeHtml(quote.message) || "—"}</p>
        <p><a href="${process.env.SITE_URL || "https://drokex.com"}/mi-cuenta/cotizaciones/proveedor">Ver en el dashboard</a></p>
      `,
    });
  } catch {}
}
  ...
  try {
    await getTransporter().sendMail({
      ...
      html: `
        ...
        <p><strong>Nota del proveedor:</strong> ${escapeHtml(quote.providerNote) || "—"}</p>
        <p><a href="${process.env.SITE_URL || "https://drokex.com"}/mi-cuenta/cotizaciones">Ver en tu dashboard</a></p>
      `,
    });
  } catch {}
}
